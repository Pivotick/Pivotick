...
| `position` | `'bottom-left' \| 'bottom-right' \| 'top-left' \| 'top-right'` | `'bottom-left'` | Which canvas corner it docks in. `'top-left'` shares its anchor with the contextual tool panel, which covers the legend while open. |

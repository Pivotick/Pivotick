...
- **`pic.png`** — a single thumbnail for the index grid (captured against the light theme; theme-aware thumbnails were considered and dropped as not worth it).
...
- **Thumbnail generation** — repo already has a Playwright visual-test setup (`npm run test:visual`). Prefer automating `pic.png` capture; **wait explicitly for async render before screenshotting** (notes/markdown/clusters render asynchronously). Manual capture is the fallback. A single `pic.png` per card (`scripts/capture-thumbnails.mjs` → `GalleryIndex.vue` glob) — theme-aware light/dark thumbnails were dropped as not worth the extra capture/selection machinery.
...
3. A `pic.png` thumbnail present and shown in the index grid.
...
| B3 | Custom HTML node ★ | Return arbitrary HTML per node | `render.renderNode(node) => HTMLElement\|string` | The "hackable" proof. Shipped as `custom-html-node` (team-card demo) once the `renderNode` measurement bug was fixed. Note: `renderNode` content must be **self-sizing** (`inline-flex`/`inline-block`) — a block-level element stretches to fill the measured `foreignObject`. |
...
- **Phase 8 — `renderNode` fix + B3 (last). ✅ Done.** `render.renderNode` (custom HTML nodes) was broken: the renderer measured the returned element with `getBoundingClientRect()` in a one-shot `requestAnimationFrame` that fires while the graph's `.zoom-layer` is still `display:none` (`_pivotick.scss`), so it locked the `foreignObject` at `0×0` and never retried — invisible for every user, not just docs. **Fixed** in `NodeDrawer.ts` with a bounded frame-retry that re-measures until the content has real dimensions (i.e. once the zoom-layer is revealed). B3 shipped as `custom-html-node` with its thumbnail. `NodeStyle.html` remains a fixed-size stopgap and is untouched. **Follow-up (deferred, see §9.8):** measured size doesn't feed the collision radius for non-cluster custom nodes, so large HTML cards overlap unless the sim is tuned (B3 spreads them via `d3ManyBodyStrength`/`d3LinkDistance`).
...
Discovered during Phase 8 (B3, custom HTML node):

8. **A `renderNode` node's measured size never feeds its collision radius** (`src/renderers/svg/NodeDrawer.ts`, renderNode branch). The measured width/height only updates `node.setCircleRadius(...)` when `enableNodeExpansion` is on (for the cluster-bubble radius); a normal graph leaves the node at the default radius 10, so the force sim treats a large HTML card as a tiny circle and packs them until they overlap. And even if the radius were set, it happens in a post-reveal `rAF` after the sim has cooled, so it wouldn't re-space without a reheat. **Worked around in B3** by tuning `d3ManyBodyStrength`/`d3LinkDistance` to spread the cards. Fix: feed the measured radius into the collision force for custom nodes (and reheat once, or measure before the initial layout settles). Also note the authoring contract this surfaced — `renderNode` content must be self-sizing (`inline-flex`/`inline-block`), or a block-level element stretches to fill the measured `foreignObject`.

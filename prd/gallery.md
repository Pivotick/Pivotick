...
- **Phase 8 — `renderNode` fix + B3 (last). ✅ Done.** `render.renderNode` (custom HTML nodes) was broken: the renderer measured the returned element with `getBoundingClientRect()` in a one-shot `requestAnimationFrame` that fires while the graph's `.zoom-layer` is still `display:none` (`_pivotick.scss`), so it locked the `foreignObject` at `0×0` and never retried — invisible for every user, not just docs. **Fixed** in `NodeDrawer.ts` with a bounded frame-retry that re-measures until the content has real dimensions (i.e. once the zoom-layer is revealed). B3 shipped as `custom-html-node` with its thumbnail. `NodeStyle.html` remains a fixed-size stopgap and is untouched. **Follow-up (deferred, see `prd/library-fixes.md` #8):** measured size doesn't feed the collision radius for non-cluster custom nodes, so large HTML cards overlap unless the sim is tuned (B3 spreads them via `d3ManyBodyStrength`/`d3LinkDistance`).
...
Building the cards exposed genuine library bugs. They are **out of scope for the gallery** (the affected cards were built around them) but should be fixed in a dedicated library pass.

These have been **extracted into their own PRD — `prd/library-fixes.md`** (targeted at the `develop` branch) — where the 8 defects are listed with root cause, file/line, the card that surfaced each, and a proposed fix. Kept out of this document so the gallery PRD stays about the gallery.

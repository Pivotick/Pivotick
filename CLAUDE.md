...
`pivot.html` + `src/pivot-demo.ts` are a second dev page (`npm run dev`, then `/pivot.html`) for pivots and enrichment: a 20-node investigation graph, six providers covering the contract's distinct paths, and a toolbar whose latency / failure / ingest-veto / save knobs reach the states a well-behaved backend never produces. The save knob covers all four answers a `save` can give — writes everything, writes half, refuses, or writes under ids of its own choosing — and the last of those is the only way to see canonical-id aliases stop a re-run from duplicating what was saved. Neither file is imported by `src/index.ts`, so nothing here reaches the bundles.

`misp-modules.html` + `src/misp-modules-demo.ts` are the same surfaces against a real backend: a running [misp-modules](https://github.com/MISP/misp-modules) service. The page reads `GET /modules` and registers one pivot per expansion module — around 120 — so the panel, the narrowing controls and the rim badges can be seen under a provider count no hand-written demo reaches. Seed nodes are real public indicators keyed `type:value`, so a module returning something already on the canvas dedups against it.

The service sends no CORS headers, so the page talks to the `/misp-api` dev-server proxy declared in `vite.config.js`; point it elsewhere with `MISP_MODULES_URL` and restart. Two things about the backend are worth knowing before touching this file: every query goes out in **both** request shapes (`attribute: {type, value, uuid}` and a bare `<type>: <value>` key) because roughly a third of the catalogue only reads the older one, and misp-modules has **no count endpoint** — so the per-module pivots deliberately declare no `summarize`, and the panel shows them all as bare Runs.
